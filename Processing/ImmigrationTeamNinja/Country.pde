...
class Country { 
  ...
  void loadSVGImage() {
    ...
    // SVG Image Load
    try {
      String svgLocation = "../../iso-country-flags-svg-collection/svg/country-squared/" + this.ISO2.toLowerCase()  + ".svg";
      //println("Opening Country SVG " + svgLocation);
      PShape countryFlag = new PShape();


      countryFlag = loadShape(svgLocation);
      this.flagSVG = countryFlag;
    }
    catch (Exception e) {
      println("Invalid SVG FLag: " + ISO2);
    }
  }
  ...
  void loadPNGImage() {
    // PNG Image Load
    try {
      String pngLocation = "../../iso-country-flags-svg-collection/png/" + this.ISO2.toLowerCase()  + ".png";
      //println("Opening Country SVG " + svgLocation);
      PImage countryFlagPNG = new PImage();
      countryFlagPNG = loadImage(pngLocation);
      this.flagPNG = countryFlagPNG;
    }
    catch (Exception e) {
      println("Invalid PNG FLag: " + ISO2);
    }
  }
...
void drawCountryFlagPNG(String ISO2, float x, float y ) {

  try {

    Country currentCountry = countryHashMap.get(ISO2);
    if (currentCountry != null) {
      if (currentCountry.flagPNG != null)
        image(currentCountry.flagPNG, x, y);
    }
  }

  catch(Exception ex) {
    println("Error drawing PNG symbol " + ISO2);
  }
}
...
void drawCountryFlagSVG(String ISO2, float x, float y ) {
  try {  
    Country currentCountry = countryHashMap.get(ISO2);
    if (currentCountry != null) {
      if (currentCountry.flagSVG != null)
      {
        shape(currentCountry.flagSVG, x, y, 10, 10);
      }
    }
  }
  catch(Exception ex) {
    println("Error drawing SVG symbol " + ISO2);
  }
}
...
void load_states() {
  ...
  ima_r = new float[] { 
    0.268713718301253, 0.48976474182707, 0.620378857317446, 0.711426825542316, 0.916437519095631, 0.942560342193706, 0.98380690498014, 1
  };
  nonima_r = new float [] { 
    0.25844772967265, 0.540258711721225, 0.700633579725449, 0.83078141499472, 0.930834213305174, 0.978616684266104, 0.988648363252376, 1
  };
  immi_r = new float [] { 
    0.354698248654663, 0.443244428753303, 0.501428250473409, 0.731301687154304, 0.955339089129249, 0.96286013843866, 0.978394368306747, 1
  };
  totals = new int [] { 
    70025, 20135, 12947, 44555, 42910, 1939, 3250, 4231
  } 
  ;
...
void DebugFeatures(List<Feature> countriesFeatureList) {
  ...
  for (Feature currentFeature : countriesFeatureList) {
    ...
    if (immigrantCountry != null && immigrantCountry.ISO2 != null) {     
      ...
      //Load the PNG image for display
      ...
      float SCALE = 1.0; // each dot represents how many people?
      float dw, dh;
